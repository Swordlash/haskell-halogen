-- NOTICE
-- Vendored from https://github.com/sebastiaanvisser/clay/pull/250
...
import Clay.Common qualified as Com
import Clay.Grid
...
-- might be provided as a 'GridLine' but also as an 'Int', 'String', etc.
--
-- == Pragma
-- #pragma#
...
data GridLine
  = -- | 'Int' value.
    --
    -- __NOTE:__ 'Int' value of 0 is invalid.
    Coordinate Int
  | -- | @custom-ident@ with an optional 'Int' value.
    --
    -- __NOTE:__ 'Int' value of 0 is invalid.
    GridLineCustomIdent CustomIdentGrid (Maybe Int)
  | -- | @span@ CSS keyword with an optional @custom-ident@ and/or 'Int' value.
    --
    -- __NOTE:__ negative 'Int' or 0 are invalid.
    Span (Maybe CustomIdentGrid) (Maybe Int)
  | -- | @auto@ CSS keyword.
    Auto
  | -- | @inherit@ CSS keyword.
    Inherit
...
-- | Both @custom-ident@ and `Int` values, provided as a pair.
--
-- __NOTE:__ 'Int' value of 0 is invalid.
instance ToGridLine (CustomIdentGrid, Int) where
  toGridLine (x, y) = GridLineCustomIdent x (Just y)

-- | Both @custom-ident@ and `Int` values, provided as a pair.
--
-- __NOTE:__ 'Int' value of 0 is invalid.
instance ToGridLine (String, Int) where
...
--
-- __NOTE:__ 'Int' value of 0 is invalid.
instance ToGridLines2 Int where
...
-- | One time both a @custom-ident@ and 'Int' values, provided as a pair.
--
-- __NOTE:__ 'Int' value of 0 is invalid.
instance ToGridLines2 (CustomIdentGrid, Int) where
  toGridLines2 = toGridLines2 . toGridLine

-- | One time both a @custom-ident@ and 'Int' values, provided as a pair.
--
-- __NOTE:__ 'Int' value of 0 is invalid.
instance ToGridLines2 (String, Int) where
  toGridLines2 = toGridLines2 . toGridLine
...
--
-- __NOTE:__ 'Int' value of 0 is invalid.
instance ToGridLines4 Int where
...
--
-- __NOTE:__ 'Int' value of 0 is invalid.
instance ToGridLines4 (CustomIdentGrid, Int) where
  toGridLines4 = toGridLines4 . toGridLine

-- | One time both a @custom-ident@ and 'Int' values, provided as a pair.
--
-- __NOTE:__ 'Int' value of 0 is invalid.
instance ToGridLines4 (String, Int) where
  toGridLines4 = toGridLines4 . toGridLine
...
--
-- * an 'Int' value of 0
-- * a pair with an 'Int' component of value 0
-- * a 'span_' function provided with an 'Int' value of 0 or negative
-- * a 'span_' function provided with a pair value with
-- an 'Int' component of 0 or negative.
...
instance Slash Int TwoGridLines where
...
instance Slash (CustomIdentGrid, Int) TwoGridLines where
  x // y = TwoGridLines (toGridLine x) (toGridLine y)

instance Slash (String, Int) TwoGridLines where
  x // y = TwoGridLines (toGridLine x) (toGridLine y)
...
--
-- __NOTE:__ negative 'Int' or 0 values are invalid.
...
--
-- __NOTE:__ negative 'Int' or 0 values are invalid.
instance ToSpan (String, Int) where
...
instance Val GridLine where
  ...
  value Unset = "unset"
  value (Coordinate x) = value $ fromIntegral x
...
-- - An 'Int' value of 0 is provided.
-- - A negative 'Int' value is provided for a 'Span' constructor.
--
