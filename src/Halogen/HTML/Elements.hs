...
element en props htmls =
  HTML $ VDom.Elem Nothing en (coerce props) (fmap (.unHTML) htmls)
...
elementNS ns en props htmls =
  HTML $ VDom.Elem (Just ns) en (coerce props) (fmap (.unHTML) htmls)
...
withKeys ctor props children =
  case ctor props [] of
    HTML (VDom.Elem x y z _) -> HTML (VDom.Keyed x y z (coerce children))
    h -> h
