...
import qualified Halogen.VDom.Machine as V
import qualified Web.Event.Event as DOM
import qualified Data.Map.Strict as M
import Halogen.VDom.DOM.Monad
import Data.Primitive
import Data.Foreign
import qualified Halogen.VDom.Utils as Util
...
buildProp emit el = renderProp
  ...
  patchProp state ps2 = do
    events <- newMutVar mempty
    let
      PropState { events = prevEvents, props = ps1 } = state
  ...
  applyProp events _ _ v =
    case v of
      ...
      Handler evty@(DOM.EventType ty) f -> do
         M.lookup ty <$> readMutVar events >>= \case
          Just handler -> do
