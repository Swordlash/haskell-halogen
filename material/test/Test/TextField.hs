...
spec m = do
  it "raises what is typed, one keystroke at a time" $ runPage $ do
  ...
  it "keeps the label up while a cleared field has focus" $ runPage $ do
    ui <- mount m HMTF.textField HMTF.emptyTextFieldSpec {HMTF.label = Just "Name"}
    input <- find ui "input"
  ...
  it "opens an outlined field's notch around a preset value" $ runPage $ do
    ui <-
      mount
        m
        HMTF.textField
