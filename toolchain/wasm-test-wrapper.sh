...
# A suite built with hspec-halogen is a reactor that exports
# hs_start, and runs in headless Chromium. It gets none of the Node test
# globals below, which would make Playwright's own process look like a page.
if node -e '
...
' "$wasm_binary"; then
  node "$(dirname "$0")/browser-test-runner.mjs" \
    "$wasm_binary" \
